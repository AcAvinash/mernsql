...
const Card: React.FC<CardProps> = ({ id, title, image, description, price, rating, tag, path, gst, shipCost }) => {
    ...
    const handleAddToCart = async (event: any) => {
        ...
        try {
            const response = await fetch(url, {
                ...
                body: JSON.stringify({ id: id, userId: uid, count: 1, title: title, price: price, path: path, gst: gst, shipCost: shipCost }),
            });
